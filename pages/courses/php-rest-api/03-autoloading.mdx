...
So let's implement just that. And using the new PHP 8.1 function syntax, we can make it look quite nice.
...
class Psr4Autoloader
{
    ...
    public function register(): void
    {
        spl_autoload_register($this->registerClass(...));
    }
    ...
    private function registerClass(string $className): void
    {
        ...
        $file = DIRECTORY_SEPARATOR . str_replace('\\', DIRECTORY_SEPARATOR, $realClass) . '.php';
...
To recap, we made a private `registerClass` function which takes a `$className`, like `\Api\CoolNamespace\MyClass`.
...
so we use a special constant `DIRECTORY_SEPARATOR` to let PHP decide what slashes to use.

Then we give this function to the `spl_autoload_register` function as the `$callback` argument.
...
The last thing that's left to do is write our first and last `require_once` statement ever. In `index.php` require the autoloader file.
...
<ChapterNavigator prev="/courses/php-rest-api/02-postman.mdx" next="/courses/php-rest-api/04-database" />
